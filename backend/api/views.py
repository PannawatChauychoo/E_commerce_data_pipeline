...
def check_agm_output_exists():
    ...
    """
    try:
        data_source_path = ROOT / "data_pipeline" / "data_source" / "agm_output"
        ...
        run_dirs = [
            ...
            if d.is_dir() and d.name.startswith("run_time=")
        ]
...
def get_latest_csv_metrics():  # pyright: ignore
    """
...
class StartSimulationView(APIView):
    ...
    def post(self, request):
        ...
        if continue_existing:
            print(
                "Continue mode requested - WalmartModel will handle continuation logic"
            )
...
class FileListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return JsonResponse({"runs": files, "total_runs": len(files)})
        except Exception as e:
            return JsonResponse(
                {
...
class BulkDownloadView(APIView):
    ...
    def get(self, request, run_id: str):
        try:
            ...
            if not csv_files:
                return JsonResponse(
                    {"error": "No CSV files found for this run"}, status=404
                )
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="simulation_{run_id}.zip"'
            )
            ...
            return JsonResponse(
                {"error": f"Failed to create zip file: {str(e)}"}, status=500
            )
