...
def save_distributions_to_npz(kde_dict: dict, out_path: str | Path):
    ...
    for category, obj in kde_dict.items():
        ...
        container[prefix + "price_dist_type"]    = np.array(obj["price_dist_type"], dtype="U10")
        container[prefix + "quantity_dist_type"] = np.array(obj["quantity_dist_type"], dtype="U10")
...
def create_product_price_table():
    ...
    walmart_transactions = read_and_process_transaction_data(wd + '/../data_source/Walmart_transactions.csv')
    ...
    save_distributions_to_npz(distribution_specs, wd + '/../data_source/category_distributions')
    
    print(f'specs save successfully at ../data_source/category_distributions.npz')
