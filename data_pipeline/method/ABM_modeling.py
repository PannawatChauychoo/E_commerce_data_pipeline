...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("./logs/ABM_modeling.log"), logging.StreamHandler()],
)
...
class Cust1(Serialization, Agent):
    ...
    def __init__(
        ...
        """
        Initialize the customer agent based on walmart customer data. Or load previous agents
        """
        super().__init__(unique_id, model)
        self.unique_id = unique_id
        self.segment_id = int(
            np.random.choice(
                list(segments_dist.keys()), size=1, p=list(segments_dist.values())
            )[0]
        )
        ...
        for key, value in cat_dist[self.segment_id].items():
            ...
            if key in demographic:
                if key == "age":
                    age_range = str(
                        np.random.choice(
                            list(value.keys()), size=1, p=list(value.values())
                        )[0]
                    )
        ...
        assert len(demographic_list) + len(purchase_behavior_list) == len(
            demographic
        ) + len(
            purchase_behavior
    ...
    def _calculate_budget(self) -> float:
        ...
        if len(list(self.purchase_history.values())) < 5:
            ...
            past_budget = [
                float(x[1]) * float(x[2])
                for i in self.purchase_history.values()
                for x in i
            ]
    ...
    def get_category_preference(self):
        ...
        return str(
            np.random.choice(
                list(self.product_category.keys()),
                size=1,
                p=list(self.product_category.values()),
            )[0]
        )
...
class Cust2(Serialization, Agent):
    ...
    _kde_attrs = ["unit_price"]
    _dict_attrs = ["product_line", "quantity", "date"]
    ...
    def __init__(
        ...
        New behavior attributes compared to cust1:
            - date
            - quantity
            - unit_price
        """
        ...
        assert cat_num + num_num == len(commerce_demographic_table) + len(
    ...
    def get_quantity(self) -> int:
        ...
        quantity = int(
            np.random.choice(
                list(self.quantity.keys()), size=1, p=list(self.quantity.values())
            )[0]
        )
        return quantity
    ...
    def get_category_preference(self):
        ...
        return str(
            np.random.choice(
                list(self.product_line.keys()),
                size=1,
                p=list(self.product_line.values()),
            )[0]
        )
    ...
    def get_mostcommon_date(self, top_date: int = 3) -> list[str]:
        ...
        return [
            x[0]
            for x in sorted(self.date.items(), key=lambda x: x[1], reverse=True)[
                0:top_date
            ]
        ]
...
class Product(Serialization, Agent):
    def __init__(
        ...
        self.annual_demand = avg_quantity * 52

        self.lead_days = max(int(np.random.normal(7, 2, 1)[0]), 1)
        self.ordering_cost = max(float(np.random.normal(20, 5, 1)[0]), 1)
        self.holding_cost_per_unit = max(
            float(np.random.normal(0.10, 0.02, 1)[0]), 0.01
        )
        self.EOQ = np.sqrt(
    ...
    def record_sales(self, quantity):
        """Record daily sales (quantity * unit_price)."""
...
def sample_from_distribution(dist, dist_type, n_samples=1) -> float:
    try:
        if dist_type == "kde":
            ...
            logger.debug("Sampling from normal distribution:")
            logger.debug(
                f"Distribution parameters: loc={dist.kwds.get('loc', 0)}, scale={dist.kwds.get('scale', 1)}"
            )
            samples = dist.rvs(size=n_samples)
            logger.debug(f"Generated samples: {samples}")

        x = samples[0]
        return 1.0 if math.isnan(x) or x < 1 else x

    except Exception as e:
        logger.error("Error sampling from distribution:")
...
def getting_segments_dist(
    path,
) -> tuple[dict[int, float], dict[int, dict[str, float]], dict[int, dict[str, float]]]:
    """
    ...
    assert (
        "product_category" in segments_cat_dist[1]
        or "product_line" in segments_cat_dist[1]
    ), "New product category or product line not found in segments_cat_dist"

    return segments_dist, segments_cat_dist, segments_num_dist


def map_cutomerpref_to_all_categories(
    segments_cat_dist: dict,
    ...
    for k in main_key:
        sub_key = [
            ...
            for x in price_table["category_path"]
            if x.startswith(k)
        ]
    ...
    for segment_id, cat_dist in segments_cat_dist.items():
        ...
        if "product_category" in lower_cat_dist or "product_line" in lower_cat_dist:
            ...
            prefs = lower_cat_dist.get("product_category", {}) or lower_cat_dist.get(
                "product_line", {}
            )
            new_prefs = {}
    ...
    assert len(final_categories) == len(
        keys_recorded
    ), "Final categories do not match recorded categories"
...
def get_itinerary_category(category: str, item_list: list) -> list[Product]:
    ...
    return [
        x
        for x in item_list
        if (match := process.extractOne(category, [x.product_category]))
        and match[1] > 80
    ]
