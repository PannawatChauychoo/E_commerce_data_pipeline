...
def setup_database():
    ...
    with open('/Users/macos/Personal_projects/Portfolio/Project_1_Walmart/Walmart_sim/backend/database/schema.sql', 'r') as f:
        schema_sql = f.read()
...
def load_customers_lookup(conn, table="customers_lookup", schema="walmart"):
    ...
    cur.execute(f"""INSERT INTO {schema}.{table} (external_id, cust_type, segment_id)
    SELECT unique_id, 'Cust1', segment_id FROM cust1_demographics;""")
    
    cur.execute(f"""INSERT INTO {schema}.{table} (external_id, cust_type, segment_id)
    SELECT unique_id, 'Cust2', segment_id FROM cust2_demographics;""")
